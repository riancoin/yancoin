...
def main():
    ...
    tmpdir = "%s/riancoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and riancoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "riancoind"]) is not None:
            print("%sWARNING!%s There is already a riancoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "riancoinD" not in os.environ:
        os.environ["RIANCOIND"] = build_dir + '/src/riancoind' + exeext
        os.environ["RIANCOINCLI"] = build_dir + '/src/riancoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `riancoin-cli help` (`rpc_interface.txt`).
