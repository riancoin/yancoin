riancoin Core version 0.13.2 is now available from:

  <https://download.riancoin.org/riancoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/riancoin-project/riancoin/issues>
...
updates will be released anymore. Without security updates, using a riancoin
...
ECDSA signatures inside riancoin transactions now use validation using
...
Previous versions of riancoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of riancoin Core was to raise the default minimum
relay fee.

riancoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
riancoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running riancoind without having to do any manual configuration.
...
riancoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. riancoin Core will relay transactions with
...
entirely in riancoin Core 0.13, and it is at this time undecided whether the
...
riancoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
riancoin Core automatically creates a hidden service to listen on, without
manual configuration. riancoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if riancoin Core is listening, and
...
riancoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals riancoin
Core to use floating fees. By default, riancoin Core will use floating
fees.
...
(default: `0.0002` RAN/kB).

At all times, riancoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) RAN.
Furthermore, riancoin Core will never create transactions paying less than
...
`riancoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /riancoinrpc>
        ...
        # optional bypass riancoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< riancoinrpc:<password>
    </Location>
...
Restarting riancoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in riancoin.conf
...
riancoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/riancoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The riancoin Core code base has
...
- `riancoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `riancoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the riancoin scripting system that in combination with
...
riancoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of riancoin Core.
...
The two phases are distinct in the riancoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming riancoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for riancoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each riancoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of riancoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of riancoins being spent is exactly
...
- **Increased security for multisig:** riancoin addresses (both P2PKH addresses
  ...
  believed to be provided by riancoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of riancoin's current ledger (called the UTXO set),
  ...
  note that riancoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  riancoin users to individually opt-in to almost any change in the riancoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and riancoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the riancoin
client to riancoin Core 0.13.2 to signal for segwit support. When a
...
Since riancoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `riancoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `riancoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
